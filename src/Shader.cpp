...
GLuint ShaderProgramCreateFromFile(const char *fileName, int type)
{
	...
	if (type & ST_TESSCONTROL)
	{
		if ((shader = glCreateShader(GL_TESS_CONTROL_SHADER)) == 0)
		{
			LOG_ERROR("Creating tesselation control shader fail (%d)\n", glGetError());
			glDeleteProgram(program);
			return 0;
		}

		snprintf(shaderName, 0xFF, "%s.tc", fileName);
	...
	if (type & ST_TESSEVAL)
	{
		if ((shader = glCreateShader(GL_TESS_EVALUATION_SHADER)) == 0)
		{
			LOG_ERROR("Creating tesselation evaluation shader fail (%d)\n", glGetError());
			glDeleteProgram(program);
			return 0;
		}

		snprintf(shaderName, 0xFF, "%s.te", fileName);
